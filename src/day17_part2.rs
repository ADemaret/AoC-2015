...
fn get_answer(input: &str, total: usize) -> usize {
    ...
    let mut nbr_containers = Vec::new();
    for p in possibilities {
        if get_total(&containers, &p) == total {
            //val += 1;
            let nbr = p.iter().filter(|&x| *x > 0).count();
            nbr_containers.push(nbr);
            // println!("bonne combinaison : {:?} en {} containers", p,nbr);
        }
    }
    let min_containers = nbr_containers.iter().min().unwrap();
    nbr_containers
        .iter()
        .filter(|&x| *x == *min_containers)
        .count()
}
